...
def tf_calculation(file_dir):
    ...
    for filename in sorted(os.listdir(file_dir)):
        if filename.endswith(".wrd"):
            ...
            tf_file.to_csv(file_dir+'/tf/'+filename.replace('.wrd','_tf')+".csv",header="False",index="False",sep=",")
            tf_all_words_per_file = copy.deepcopy(all_words)
            for i in tf_in_file.keys():
                tf_all_words_per_file[re.sub(r"\,\s\'.{1,7}\.\w{3}\'","",i)] = tf_in_file[i]
            f = open(file_dir+"/tf_txt/"+"tf_vectors_"+filename+".txt","a")
            f.write(str(tf_in_file))
            tf_all_words_all_files = pd.concat([tf_all_words_all_files,pd.DataFrame(tf_all_words_per_file,index=[0])],axis=0)
    tf_all_words_all_files.to_csv(file_dir+"/all_tf.csv",header="True",index="True",sep=",")
    return all_df
...
def tfidf_calculation(all_df,file_dir,N):
    ...
    for filename in sorted(os.listdir(file_dir)):
        if filename.endswith(".wrd"):
            ...
            file = open(file_dir+'/'+filename, 'rb')
            temp_df = pickle.load(file)
            temp_tf_df = pd.read_csv(file_dir+'/tf/'+filename.replace('.wrd','_tf')+".csv",header=0,index_col=0,sep=",")   
            file.close()
            tfidf_in_file = {}
            ...
            for i in range(len(temp_df)):
                for j in range(len(temp_df.iloc[0])):
                    ...
                    temp_df2.iloc[i,j] = temp_tf_df.iloc[i,j]*math.log(n/words_in_files[str(word)])
                    tfidf_in_file[str(word)] = temp_df2.iloc[i,j]
                    tfidf_all_words_per_file[str(word)] = temp_df2.iloc[i,j]
            temp_df2.to_csv(file_dir+'/tfidf/'+filename.replace(".wrd",'_tfidf')+".csv",header="False",index="False",sep=",")
            f = open(file_dir+"/tfidf_txt/"+"tfidf_vectors_"+filename+".txt","a")
            f.write(str(tfidf_in_file))
            tfidf_all_words_all_files = pd.concat([tfidf_all_words_all_files,pd.DataFrame(tfidf_all_words_per_file,index=[0])],axis=0)
    tfidf_all_words_all_files.to_csv(file_dir+"/all_tfidf.csv",header="True",index="True",sep=",")
            
            
def combine_tf_gestures(file_dir):
    filenames=[]
    all_tf = pd.read_csv(file_dir+"/all_tf.csv",header=0,index_col=0,sep=',')
    for filename in sorted(os.listdir(file_dir)):
    ...
    all_tf.to_csv(file_dir+"/all_tf_gestures.csv",header="True",index="True",sep=",")
    file_gesture = open(file_dir+"/vectors/tf_vectors_"+filename.replace("tf_vectors_W_",'')+".txt","a+")
...
def combine_tfidf_gestures(file_dir):
    filenames=[]
    all_tfidf = pd.read_csv(file_dir+"/all_tfidf.csv",header=0,index_col=0,sep=',')
    for filename in sorted(os.listdir(file_dir)):
        if filename.startswith("W"):
            filenames.append(filename.replace('W_',''))
    n=len(filenames)
    ...
    all_tfidf.to_csv(file_dir+"/all_tfidf_gestures.csv",header="True",index="True",sep=",")
    file_gesture = open(file_dir+"/vectors/tfidf_vectors_"+filename.replace("tfidf_vectors_W_",'')+".txt","a+")
...
if __name__ == "__main__":
    ...
    os.mkdir(file_dir+"/tf/")   #creating tf folder to store CSVs with TF values
    os.mkdir(file_dir+"/tf_txt/")
    os.mkdir(file_dir+"/tfidf/")    #creating tfidf folder to store CSVs with TFIDF values 
    os.mkdir(file_dir+"/tfidf_txt/")      #creating idf folder to store CSVs with IDF values
    N=vocabulary(N,file_dir)
    all_df = tf_calculation(file_dir)
    all_df.columns = range(all_df.shape[1])
    all_df.to_csv(file_dir+"/concatenated.csv",header="False",index="False",sep=",")
    all_df = pd.read_csv(file_dir+"/concatenated.csv",header=0,index_col=0,sep=",")
