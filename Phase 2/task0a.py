...
def createbands(r):
    mu = 0
    sigma = 0.25
    gaussian = lambda x : np.exp(-((x-mu)**2)/(2*sigma*sigma))
...
def createWindows(quantized_data, nomralized_data, window_length, shift_length, filename, mean, standardDeviation, component_name):
    ...
    for index, row in data.iterrows():
        ...
        while t+w < len(row):
            ...
            win_avg_amplitude = reduce(lambda a, b: a + b, normalized_windows) / len(normalized_windows) 
...
for root, dirnames, filenames in os.walk(path):
    ...
    for filename in fnmatch.filter(filenames, '*.csv'):
        ...
        normalized_data = normalizeData(data)
        quantized_data = quantizeData(normalized_data, r)
        window_data = createWindows(quantized_data, normalized_data, w, s, filename, m, std, component_name)
        #*******************
