...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 24 19:29:38 2020
...
def score_computation(name, model, latent, principle_comp):
    ...
    tempdf.to_csv('./output/NMFGestureScoreOnSimilarityMatrix.csv', sep=",")
...
def performNMF(DF_euclid,principle_comp):
    nmf_list = list(DF_euclid.columns.values)
    dfT = DF_euclid.T

    #    scaled_data = preprocessing.scale(df)
    nmf = NMF(n_components=principle_comp, init='random', random_state=0, max_iter=10000)

    nmf.fit(DF_euclid)
    nmf_data = nmf.transform(DF_euclid)

    print("\n")
    print("Latent Semantic Term - Weight Pairs: ")
    print()
    lat_semantics = display_topics(nmf, nmf_list, len(nmf.components_[0]))
    print()
    score_computation('NMF', nmf, lat_semantics, principle_comp)
    new_df = pd.DataFrame(nmf_data)
    new_df = new_df.T
    new_df.columns = dfT.columns
    new_df = new_df.T
    new_df.to_csv('./output/NMFonSimilarityMatrix.csv', sep=",")
    nmf_score = pd.DataFrame(nmf.components_)
    nmf_score.columns = nmf_list
    nmf_score.to_csv('./output/NMFScores.csv',header=True,sep=',')
...
if __name__ == "__main__":
    ...
    if int(red_choice) == 1:
        ...
        if principle_comp > len(DF_euclid) or principle_comp < 1:
            ...
            exit(1)
        if DF_euclid.min().min() < 0:
            DF_euclid -= DF_euclid.min().min()
        performNMF(DF_euclid, principle_comp)
    ...
    if int(red_choice) == 3:
        ...
        if DF_euclid.min().min() < 0:
            DF_euclid -= DF_euclid.min().min()
        # DF_euclid.to_csv("./output/Similarity_matrix1.csv", sep=',')
        performNMF(DF_euclid, principle_comp)
